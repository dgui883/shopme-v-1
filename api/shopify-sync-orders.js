...
// Vercel Hobby caps Node function duration at 10s by default (configurable
// up to 60s via project settings / Pro plan). A 20-page paginated sync can
// exceed that — see README-DEPLOY.md for the Vercel-specific note on this.
...
export default async function handler(req, res) {
  ...
  const { data: store } = await admin
    ...
    .maybeSingle();
  if (!store) return json(res, 404, { error: 'No connected Shopify store found.' });
  ...
  const { data: connection } = await admin
    ...
    .is('revoked_at', null)
    .maybeSingle();
  if (!connection) return json(res, 404, { error: 'Shopify connection not found.' });
  ...
  try {
    ...
    do {
      ...
      const resp = await fetchWithRetry(url.toString(), { headers: { 'X-Shopify-Access-Token': accessToken } });
    ...
    return json(res, 200, { success: true, synced: totalSynced });
  } catch (err) {
    return safeError(res, 'Order sync failed. It will retry automatically.', err);
  }
}
