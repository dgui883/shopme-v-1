...
function templatedMessage({ orderNumber, customerFirstName, healthReason, trackingUrl }) {
  ...
  const trackingLine = trackingUrl ? ` You can follow the latest updates here: ${trackingUrl}.` : '';
  return (
    `${greeting}\n\n` +
...
export default async function handler(req, res) {
  ...
  if (!allowed) return json(res, 429, { error: 'Too many message generations. Please try again later.' });
  ...
  try {
    ...
    if (order.shopify_stores.user_id !== profile.id && profile.role !== 'admin') {
      return json(res, 403, { error: 'You do not have access to this order.' });
    }
    ...
    return safeError(res, 'Could not generate a message right now. Please try again.', err);
  }
}
